...
class DPOTrainer(ABC):
    ...
    def __init__(
        ...
        self.reward = self.train_dataloader["chosen_reward"]
        self.rejected = self.train_dataloader["rejected_reward"]
    ...
    def _get_batch_logps(
        ...
        baseline = 0.
        chosen_values_mask = chosen_values > baseline
        rejected_values_mask = rejected_values < baseline
        loss_mask = (loss_mask.bool() * torch.cat([chosen_values_mask, rejected_values_mask], dim=0)).bool()
        action_logps = action_logps * experience.action_mask
